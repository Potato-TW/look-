...
def download_video_frames(loc,d_csv,start_idx,end_idx,rm_video):
    ...
    # for i in range(len(d_csv)):
        ...
        #converts to frames
        #command += 'ffmpeg -i %s.mp4 -y -f image2  -vframes 75 ../frames/%s-%%02d.jpg;' % (f_name, f_name)
        command += 'ffmpeg -i %s.mp4 -vf fps=25 ../frames_test/%s-%%02d.jpg;' % (f_name, f_name)
        ...
        file = Path(f'./video_test/{i}.mp4')
        if file.exists():
            with open(f'NotPrivate_video_test.txt', 'a') as f:
                f.write(file.name + '\n')
            cnt+=1
...
download_video_frames(loc='video_test',d_csv=cat_test,start_idx=0,end_idx=100000,rm_video=False)